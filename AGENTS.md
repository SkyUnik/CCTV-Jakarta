...
- Keep PiP enabled on all video elements; auto-switch must continue working during PiP by avoiding `removeAttribute("src")` and `video.load()` before the new source is set. Eagerly set `src` and `preload` attributes for synchronous native gesture playback.
